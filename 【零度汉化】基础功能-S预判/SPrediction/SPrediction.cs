...
namespace SPrediction1
{
    ...
    public static class Prediction
    {
        ...
        private static SPrediction1.Collision Collision;
        ...
        public static void Initialize(Menu mainMenu = null)
        {
            ...
            Collision = new SPrediction1.Collision();
        ...
        public static Vector2 GetArcPrediction(Obj_AI_Base target, Spell s, List<Vector2> path, float avgt, float movt, float avgp, out HitChance hc, Vector3 rangeCheckFrom)
        {
            ...
            for (int i = 1; i < path.Count; i++)
            {
                ...
                var dianaArc = new SPrediction1.Geometry.Polygon(
		...
		public static PredictionOutput GetSPrediction(this Spell s, Obj_AI_Hero t, int reactionIgnoreDelay = 0, byte minHit = 1, Vector3? rangeCheckFrom = null, float filterHPPercent = 100)
		{
			if (rangeCheckFrom == null)
				rangeCheckFrom = ObjectManager.Player.ServerPosition;
			...
			if (t == null)
			{
				return new PredictionOutput { Hitchance = HitChance.Impossible };
			}


			if (!s.IsSkillshot)
			{
				throw new InvalidOperationException("SPrediction is only for skillshots.");
				return new PredictionOutput { Hitchance = HitChance.Impossible };
			}
			...
			if (predMenu != null && predMenu.Item("PREDICTONLIST").GetValue<StringList>().SelectedIndex == 1)
			{
				...
				if (minHit > 1)
					if (pout.AoeTargetsHitCount >= minHit)
						s.Cast(pout.CastPosition);
					else return new PredictionOutput { Hitchance = HitChance.Impossible };

					return new PredictionOutput { CastPosition = pout.CastPosition, Hitchance = pout.Hitchance };
			...
			if (minHit > 1)
			{
				var pos = Aoe.GetSpredAoe(s, t, reactionIgnoreDelay, minHit, rangeCheckFrom, filterHPPercent);
				return pos;
			}
			...
			if (Monitor.TryEnter(EnemyInfo[t.NetworkId].m_lock))
			{
				try
				{
					...
					if (s.Collision && Collision.CheckCollision(rangeCheckFrom.Value.To2D(), pos, s, true, false, true))
					{
					 ...
					 return new PredictionOutput { CastPosition = pos.To3D(), Hitchance = predictedhc };
					

					Monitor.Pulse(EnemyInfo[t.NetworkId].m_lock);
					return new PredictionOutput { CastPosition = pos.To3D(), Hitchance = HitChance.Impossible };
				}
				finally
				{
					Monitor.Exit(EnemyInfo[t.NetworkId].m_lock);
		...
		public static class Aoe
        {
			...
			internal static PredictionOutput GetSpredAoe(Spell s, Obj_AI_Hero t, int reactionIgnoreDelay = 0, byte minHit = 2, Vector3? rangeCheckFrom = null, float filterHPPercent = 0)
			{
				if (!blInitialized)
					throw new Exception("Prediction is not initalized");
					...
					try
					{
						...
						switch (s.Type)
						{
							case SkillshotType.SkillshotLine:
								pos = Line.GetPrediction(t, s, t.GetWaypoints(), avgt, movt, avgp, filterHPPercent, minHit, out predictedhc, rangeCheckFrom.Value);
								break;
							case SkillshotType.SkillshotCircle:
								pos = Circle.GetPrediction(t, s, t.GetWaypoints(), avgt, movt, avgp, filterHPPercent, minHit, out predictedhc, rangeCheckFrom.Value);
								break;
							case SkillshotType.SkillshotCone:
								pos = Cone.GetPrediction(t, s, t.GetWaypoints(), avgt, movt, avgp, filterHPPercent, minHit, out predictedhc, rangeCheckFrom.Value);
								break;
						}
