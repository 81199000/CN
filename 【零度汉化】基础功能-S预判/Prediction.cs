...
namespace LeagueSharp.Common
{
    ...
    public class PredictionInput
    {
	    ...
	    /// <summary>
	    ///     The spell reference for which prediction is required
	    /// </summary>
    ...
    public static PredictionOutput GetPrediction(Obj_AI_Base unit, float delay)
		...
		public static PredictionType SetPred
		{
			get
			{
				if (SPrediction1.Prediction.predMenu.Item("PREDICTONLIST").GetValue<StringList>().SelectedIndex == 0)
				{
					return PredictionType.SPrediction;
				}
				if (SPrediction1.Prediction.predMenu.Item("PREDICTONLIST").GetValue<StringList>().SelectedIndex == 1)
				{
					return PredictionType.Common;
				}
				return PredictionType.SPrediction;
			}
		}
		...
		internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision)
        {
	        ...
	        Spell spell = input.Spellref;

	        var target = (Obj_AI_Hero) input.Unit;

			if (SetPred == PredictionType.SPrediction)
			{
				if (input.Aoe)
				{
					return spell.GetSPrediction(target, 0, 2);
				}
				return spell.GetSPrediction(target);
			}

			if (SetPred == PredictionType.Common)
			{
